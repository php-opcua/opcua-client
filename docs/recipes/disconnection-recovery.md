...
auto-retry, and `transferSubscriptions()` — plus the `ClientDisconnected`
event paired with a `ClientDisconnecting` flag (or a thrown
`ConnectionException`) to tell clean teardowns from broken ones.
...
3. `ClientDisconnected` fires (without a prior `ClientDisconnecting`,
   which is the marker that the teardown was not user-initiated).
...
`ClientDisconnected`. The event itself carries only `$client` — to
distinguish a clean `disconnect()` from a broken channel, track the
preceding `ClientDisconnecting` (which only fires on the clean path):
...
$dispatcher->addListener(ClientDisconnecting::class, function () use (&$cleanTeardown) {
    $cleanTeardown = true;
});

$dispatcher->addListener(ClientDisconnected::class, function (ClientDisconnected $e) use (&$cleanTeardown, $client, &$savedIds) {
    if ($cleanTeardown) {
        $cleanTeardown = false;
        return;
    }
