...
`$errorCode`, `$errorMessage` — the OPC UA status code from the server's
ERR response during HEL/ACK and the server's accompanying reason string.
The server explicitly refused the handshake. Common causes: protocol
version mismatch, buffer size negotiation failure, server in shutdown.
...
`$fingerprint`, `$certDer`, `$message` — the server certificate was
rejected by the trust store. The fingerprint and DER bytes let setup
tools prompt the operator to trust the certificate manually; the
message carries the human-readable rejection reason. See
...
`$nodeId`, `$expectedType`, `$givenType`, `$message` — reserved for
type-mismatch detection. Currently not thrown by the library itself;
the class exists for use by custom validation code and a planned
future static-analysis pass.
