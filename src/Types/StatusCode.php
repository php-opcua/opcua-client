...
class StatusCode
{
    ...
    public const UncertainDataSubNormal = 0x40A40000;

    public const BadAggregateInvalidInputs = 0x80D60000;

    public const BadAggregateNotSupported = 0x80D50000;
    ...
    /**
     * @deprecated Not a standard OPC UA status code. FileType methods report an unknown file handle as
     *             BadInvalidArgument (OPC UA Part 20), which is the value of this alias. Use {@see self::BadInvalidArgument}.
     */
    public const BadFileHandleInvalid = 0x80AB0000;

    /**
     * @deprecated Not a standard OPC UA status code. FileType methods report a handle not opened for the
     *             requested access as BadInvalidState (OPC UA Part 20), which is the value of this alias. Use {@see self::BadInvalidState}.
     */
    public const BadFileNotOpened = 0x80AF0000;
    ...
    /**
     * Names of the standard status codes, generated from the OPC Foundation's
     * UA-Nodeset Schema/StatusCode.csv. Keys have no InfoBits set.
     */
    private const NAMES = [
        self::Good => 'Good',
     ...
     *
     * The name comes from the code's upper 16 bits. Set low bits are appended in brackets:
     * StructureChanged, SemanticsChanged and, for DataValue InfoBits, the limit, overflow
     * and historian flags — e.g. `Good [LimitHigh, Overflow]`. Bits with no defined meaning
     * are appended as hex.
     *
     * @param int $code
     * @return string
    ...
    {
        $name = self::NAMES[$code & 0xFFFF0000] ?? null;
        if ($name === null) {
            return sprintf('0x%08X', $code);
        }

        $flags = self::describeLowBits($code);

        return $flags === [] ? $name : $name . ' [' . implode(', ', $flags) . ']';
    }
     ...
     * @return string[]
     */
    private static function describeLowBits(int $code): array
    {
        $flags = [];
        $explained = 0xC000;
        ...
        if (($code & 0x4000) !== 0) {
            $flags[] = 'SemanticsChanged';
        }

        if (self::hasDataValueInfoBits($code)) {
            $explained |= self::INFO_TYPE_MASK | self::LimitConstant | self::Overflow
                | self::HistorianPartial | self::HistorianExtraData | self::HistorianMultiValue;

            $limit = self::limit($code);
            if ($limit !== DataValueLimit::None) {
                $flags[] = 'Limit' . $limit->name;
            }
            if (self::isOverflow($code)) {
                $flags[] = 'Overflow';
            }
            ...
            if ($source === self::HistorianCalculated) {
                $flags[] = 'HistorianCalculated';
            } elseif ($source === self::HistorianInterpolated) {
                $flags[] = 'HistorianInterpolated';
            }

            foreach (['HistorianPartial' => self::HistorianPartial, 'HistorianExtraData' => self::HistorianExtraData, 'HistorianMultiValue' => self::HistorianMultiValue] as $flag => $bit) {
                if (($code & $bit) !== 0) {
                    $flags[] = $flag;
                }
            }
        ...
        $unexplained = $code & 0xFFFF & ~$explained;
        if ($unexplained !== 0) {
            $flags[] = sprintf('0x%04X', $unexplained);
        }
